...
# スマホ最適化 / ページ移動なし（上部セグメントで切替）/ CSV保存 / 見返し&エクスポート
...
.progress{ display:flex; gap:6px; margin:4px 0 10px 0; }
.step{ flex:1; height:8px; border-radius:999px; background:#efe9ff; overflow:hidden; }
.step .fill{ height:100%; background:#b3a8ff; width:100%; }
</style>
"""
...
if "view" not in st.session_state:
    st.session_state.view = "CBT"  # CBT / リフレク / 履歴 / エクスポ
...
def support_from_levels(distress: Optional[int]=None, lonely: Optional[int]=None):
    if distress is not None and distress >= 7:
        ...
        companion("🤝", "この瞬間、あなたは一人きりじゃない。",
                  "画面越しでも、言葉は届く。小さく息を吐こう。")
    else:
        companion("🌙", "書けた分だけ、ちゃんと前進。",
                  "短くても十分。続けることがいちばんの力。")

# ---------------- Header ----------------
greet = "こんばんは" if (datetime.now().hour >= 18 or datetime.now().hour < 4) else "こんにちは"
st.markdown(
    f"""
...
# ---------------- Segmented control（ページ移動なし） ----------------
with st.container():
    st.markdown('<div class="segment">', unsafe_allow_html=True)
    st.session_state.view = st.radio(
        "モード", ["CBT", "リフレク", "履歴", "エクスポ"],
        index=["CBT","リフレク","履歴","エクスポ"].index(st.session_state.view),
...
def section_cbt():
    ...
    # mini progress
    st.markdown("<div class='progress'><div class='step'><div class='fill'></div></div></div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def section_history():
    ...
    if df.empty:
        ...
        for _, r in view.head(50).iterrows():
            ...
            try:
                b = int(r.get("distress_before", 0))
                a = int(r.get("distress_after", 0))
            ...
            # ✅ バックスラッシュ無し（以前のSyntaxError修正ポイント）
            if tags:
...
# --------- Render selected view (single page) ---------
if st.session_state.view == "CBT":
    section_cbt()
elif st.session_state.view == "リフレク":
    section_reflect()
elif st.session_state.view == "履歴":
    section_history()
else:
    section_export()
