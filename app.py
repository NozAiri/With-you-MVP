...
def db_create_user(group_id: str, handle_norm: str, class_info: Dict[str, str]) -> Tuple[bool, str]:
    ...
    try:
        ref.create({
            ...
            "class_info": class_info,  # クラス情報を追加
        })
        return True, ""
    except Exception:
...
# ================== 気分の絵文字マッピング ==================
MOOD_EMOJI_MAP = {
    "😟": {"label": "つらい", "score": 2, "color": "#ff6b6b"},
    "😐": {"label": "ふつう", "score": 1, "color": "#ffd93d"},
    "🙂": {"label": "まあまあ", "score": 0, "color": "#6bcf7f"},
}
...
def login_register_ui() -> bool:
    ...
    st.divider()
    st.markdown("**クラスのパスワードを入力してください**")
    st.caption("例：1年A組2025、2年B組2025 など")
    group_pw = st.text_input(
        "パスワード", 
        key="inp_group_pw", 
        label_visibility="collapsed", 
        placeholder="例：1年A組2025"
    )
    ...
    if st.button(btn_label, type="primary", use_container_width=True, disabled=disabled, key="btn_go"):
        ...
        if mode == "REGISTER":
            ...
            st.session_state.flash_msg = f"登録が完了しました。{class_info['class_id']}へようこそ！"
            st.rerun()
        else:
            if not db_user_exists(gid, handle_norm):
                st.error("まだ登録がありません。「はじめての人（登録）」から設定してください。")
                st.stop()
...
def view_share():
    ...
    memo = st.text_area(
        "今日のようす（任意・先生に伝えたいこと）",
        height=100,
        placeholder="例：テストがあった / 体育で疲れた / 特になし",
        key="share_memo"
    )
    ...
    if st.button(label, type="primary", disabled=disabled, key="share_send"):
        ...
        payload = {
            ...
            "class_info": class_info,  # 【重要】クラス情報を追加
            "payload": {
                "mood": mood,
            ...
            "risk_level": risk_level,  # 【重要】リスクレベルを追加
            "anonymous": True
        }
...
def view_consult():
    ...
    msg = st.text_area(
        ...
        value="", 
        key="c_msg",
        placeholder="困っていること、話したいことを自由に書いてください。"
    )
...
def view_note():
    ...
    trigger_text = text_card(
        "🫧 Step 2：その気持ちは、どんなことがきっかけだった？", 
        "「○○があったからかも」「なんとなく○○って思ったから」など自由に。", 
        "cbt_trigger"
    )
    auto_thought = text_card(
        "💭 Step 3：そのとき、頭の中でどんな言葉がよぎった？", 
        "心の中でつぶやいた言葉やイメージをそのまま書いてOK。", 
        "cbt_auto"
    )
    ...
    reason_against = text_card(
        "🔍 Step 5：そうでもないかもと思う理由はある？", 
        "「でも、こういう面もあるかも」も書いてみよう。", 
        "cbt_against", 
        height=100
    )
    alt_perspective = text_card(
        "🌱 Step 6：もし友だちが同じことを感じていたら、なんて声をかける？", 
        "自分のことじゃなく「友だち」のこととして考えてみよう。", 
        "cbt_alt"
    )
...
""", unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        if not studies: 
            ...
            pie_agg = df.groupby("subject")["minutes"].sum().reset_index().sort_values(
                ...
                pie = (
                    ...
                    .encode(
                        ...
                        color=alt.Color(
                            field="subject", 
                            type="nominal", 
                            legend=alt.Legend(title="科目")
                        ),
                        tooltip=[
                            alt.Tooltip("subject:N", title="科目"), 
                            alt.Tooltip("minutes:Q", title="合計分")
                        ]
                    )
